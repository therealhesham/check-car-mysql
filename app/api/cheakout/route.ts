...
export async function POST(req: NextRequest): Promise<NextResponse<ApiResponse>> {
  try {
    ...
    // Check for existing checkout record based on operation type
    if (fields['نوع العملية'] === 'دخول') {
      const existingExitRecord = await prisma.contracts.findFirst({
        where: {
          contract_number: contractNumber,
          operation_type: 'خروج',
        },
      });
      if (!existingExitRecord) {
        return NextResponse.json(
          {
            success: false,
            error: 'لا يوجد سجل خروج لهذا العقد',
          },
          { status: 409 }
        );
      }
    } else if (fields['نوع العملية'] === 'خروج') {
      const existingExitRecord = await prisma.contracts.findFirst({
        where: {
          contract_number: contractNumber,
          operation_type: 'خروج',
