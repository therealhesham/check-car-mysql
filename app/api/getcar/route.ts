...
// جلب قائمة اللوحات من جدول CarsDetails
export async function GET(req: NextRequest) {
  try {
    // جلب جميع السجلات من CarsDetails التي تحتوي على لوحة
    const records = await prisma.carsDetails.findMany({
      where: {
        plate: {
          not: null,
        },
    ...
    // تحويل البيانات للهيكل المطلوب
    const plates = records.map((record) => ({
      id: record.id,
      fields: {
        Name: String(record.plate), // اللوحة تظهر في قائمة الاختيار
      },
      // نُخزن معلومات السيارة مع اللوحة لاستخدامها لاحقًا
      carInfo: {
    ...
    return NextResponse.json({
      success: true,
      message: 'لوحات السيارات تم جلبها بنجاح!',
      results: plates,
    });
  } catch (error: any) {
    console.error('Error fetching plates from CarsDetails:', error);
    return NextResponse.json(
      {
        success: false,
        message: 'فشل في جلب اللوحات من تفاصيل السيارات.',
        error: error.message || String(error),
      },
      { status: 500 }
    );
