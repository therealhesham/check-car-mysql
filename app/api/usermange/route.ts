...
export async function POST(req: NextRequest) {
  try {
    ...
    const branchRegex = /^[ء-ي\s,]+$/; // يسمح بالحروف العربية، المسافات، والفواصل
    if (!branchRegex.test(data.fields.branch.trim())) {
      return NextResponse.json(
        {
          success: false,
          message: 'أسماء الفروع يجب أن تحتوي على حروف عربية، مسافات، وفواصل فقط.',
          error: 'INVALID_BRANCH_FORMAT',
        },
        { status: 400 }
      );
    ...
    // التحقق من أن الفروع المختارة موجودة في قاعدة البيانات
    const selectedBranches = data.fields.branch.split(',').map((branch) => branch.trim());
    const validBranches = await prisma.branches.findMany({ select: { branch_name: true } });
    const validBranchNames = validBranches.map((branch) => branch.branch_name);
    const invalidBranches = selectedBranches.filter((branch) => !validBranchNames.includes(branch));
    if (invalidBranches.length > 0) {
      return NextResponse.json(
        {
          success: false,
          message: 'تم اختيار فروع غير صالحة: ' + invalidBranches.join(', '),
