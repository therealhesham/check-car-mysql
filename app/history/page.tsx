...
import { FaHistory, FaSearch, FaChevronDown, FaDownload, FaCopy } from 'react-icons/fa';
import Image from 'next/image';
import { useRouter } from 'next/navigation';
import Lightbox, { ZoomRef } from 'yet-another-react-lightbox';
import Counter from 'yet-another-react-lightbox/plugins/counter';
...
export default function HistoryPage() {
  ...
  const [isGeneratingPDF, setIsGeneratingPDF] = useState<string | null>(null); // معرف السجل
  const datetimeRef = useRef<any>(null); // مرجع لمكون Datetime
  // history.tsx (في قسم تعريف الحالات useState، مع الحالات الأخرى)
...
const [datetimeFilter, setDatetimeFilter] = useState<string>(''); // Keep this for the single field
  ...
  const operationTypes = ['دخول', 'خروج'];
  // history.tsx (في قسم تعريف الدوال، مع الدوال الأخرى)
const copyToClipboard = async (text: string | number | null, recordId: string) => {
  // تحويل القيمة إلى نص إذا لم تكن كذلك
  const textToCopy = text !== null && text !== undefined ? String(text) : '';
  if (!textToCopy) return; // لا تحاول نسخ نص فارغ

  try {
    await navigator.clipboard.writeText(textToCopy);
    setCopiedContractId(recordId); // تعيين معرف السجل الذي تم نسخه
    // إعادة تعيين الحالة بعد 2 ثانية لإزالة تغذية الراجعة
    setTimeout(() => setCopiedContractId(null), 2000);
  } catch (err) {
    console.error('فشل في نسخ النص: ', err);
    setError('فشل في نسخ رقم العقد إلى الحافظة.');
  }
};
...
<tbody>
  {filteredRecords.map((record) => {
    ...
    return (
      ...
      >
        <td className="py-3 px-4 text-right text-gray-900 dark:text-gray-100">
  <div className="flex items-center justify-end"> {/* محاذاة المحتوى لليمين */}
    {record.contract_number && (
      <button
        onClick={(e) => {
          e.stopPropagation();
          copyToClipboard(record.contract_number, record.id);
        }}
        className="text-gray-500 dark:text-gray-400 hover:text-blue-500 dark:hover:text-blue-400 focus:outline-none mr-2" // margin left للمسافة
        title={copiedContractId === record.id ? "تم النسخ!" : "نسخ إلى الحافظة"}
        aria-label={`نسخ رقم العقد ${record.contract_number} إلى الحافظة`}
      >
        {copiedContractId === record.id ? (
